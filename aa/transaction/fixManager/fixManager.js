...
function getChart(fixManagerMaxaccept) {
	...
	var datayywc = [];//已完成工单数
	var dataydpf = [];//待派发工单数量
	var datayno = [];//维护工单_不满意数量
	
	getTotalCount(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId, orderChannel);
...
//查询受理工单数量
function getTotalCount(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId, orderChannel){
	var test0 = [];
	$.ajax({
		...
		success : function(data) {
			...
			//依次调用方法，传递获取所有需要数据
			getDataAccess(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId, orderChannel);
		}
	});
}
...
//查询已完结工单数量
function getDataAccess(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId, orderChannel){
	var test1 = [];
	$.ajax({
		...
		success : function(data) {
			...
			getKFreview(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId, orderChannel);
		}
	});
}

//查询待客服回访工单数量
function getKFreview(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId, orderChannel){
	var test3 = [];
	$.ajax({
		...
		success : function(data) {
			...
			getWaitSendOrder(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId, orderChannel);
		}
	});
}

//查询待派发工单数量
function getWaitSendOrder(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId, orderChannel){
	var test4 = [];
	$.ajax({
		url : webpath + "/fixPerson/getWaitSendOrder.action?beginTime="+beginTime+"&endTime="+endTime+"&userId="+userId+"&managerId="+managerId+"&orderChannel="+orderChannel,
		cache: false,//不保存缓存
		type : "GET",
		dataType : "json",
		success : function(data) {
			...
			getDataUnAccess(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId);
		}
	});
}

//查询待维护工单数量
function getDataUnAccess(beginTime, endTime, datax, datay, dataydpf, dataydwh, dataywhf, datayywc, dateArr, userId, managerId, orderChannel){
	var test2 = [];
	$.ajax({
		...
		success : function(data) {
			...
			//绘制HighCharts图表
			start(datax, datay, dataydpf, dataydwh, dataywhf, datayywc);
		}
	});
}
...
//Highcharts线性图表
function start(datax, datay, dataydpf, dataydwh, dataywhf, datayywc) {
	$('#line').highcharts({
		chart : {
			renderTo: 'container',
			type : 'column'
